...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE write_data[2] = {reg + 2, val}; //send 2 bytes total
	//write_data[0] = reg + 2;
	//write_data[1] = val;

	int return_code = XSpi_Transfer(&SpiInstance, write_data, NULL, 2);

	if (return_code != 0){
		xil_printf("One reg write error, code: %d \n", return_code);
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE write_data[nbytes + 1]; //send n bytes + status byte
	write_data[0] = reg + 2;
	...
	int return_code = XSpi_Transfer(&SpiInstance, write_data, NULL, nbytes + 1);

	if (return_code != 0){
		xil_printf("Multiple reg write error, code: %d \n", return_code);
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int return_code = XSpi_Transfer(&SpiInstance, read_data, read_data, 2);

	if (return_code != 0){
		xil_printf("One reg read error, code: %d \n", return_code);
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	if (return_code != 0){
		xil_printf("Multiple reg read error, code: %d \n", return_code);
	}
